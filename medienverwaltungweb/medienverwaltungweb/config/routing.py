...
def make_map(config):
    ...
    map.connect('/json', controller='medium', action='list_gallery', json=True)
    map.connect('/{controller}/{action}')
    map.connect('/m/{controller}/{action}', mobile=True)
    map.connect('/json/{controller}/{action}', json=True)
    map.connect('/{controller}/{action}/{id}')
    map.connect('/m/{controller}/{action}/{id}', mobile=True)
    map.connect('/json/{controller}/{action}/{id}', json=True)
